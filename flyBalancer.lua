...
local THRUST_GRADIENT = (MAX_THRUST - MIN_THRUST) / 20
local ACTUAL_THRUST = (MAX_THRUST - MIN_THRUST) / 2

local MAX_PITCH = 90
local MIN_PITCH = -90
local PITCH_GRADIENT = (MAX_PITCH - MIN_PITCH) / 20 
...
local YSpeed =speedgroup.addText({20,0}, meta.motionY.."b/s")
speedgroup.addText({40,0}, "South-North")
local ZSpeed =speedgroup.addText({60,0}, meta.motionZ.."b/s")
speedgroup.addText({80,0}, "West-East")
local XSpeed = speedgroup.addText({100,0}, meta.motionX.."b/s")
speedgroup.addText({120,0}, "Thrust")
local ThrustSpeed = speedgroup.addText({140,0}, ACTUAL_THRUST.."%")
speedgroup.addText({160,0}, "Pitch")
local PitchSpeed = speedgroup.addText({180,0}, ACTUAL_PITCH.."degrees")
